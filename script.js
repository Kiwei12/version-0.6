// --- Login yoki ro'yxatdan o'tgandan keyin tugmani yashirish va ism ko'rsatish ---
document.addEventListener('DOMContentLoaded', function() {
    const loginForm = document.getElementById('login-form');
    const registerForm = document.getElementById('register-form');
    const modal = document.getElementById('login-register-modal');
    const loginRegisterBtn = document.getElementById('login-register-btn');
    // Foydalanuvchi nomini ko'rsatish va profil modalini ochish uchun joy
    ...
    // --- Login flag tekshirish va UI ni moslash ---
    if (localStorage.getItem('loggedIn') === 'true') {
        if (loginRegisterBtn) loginRegisterBtn.style.display = 'none';
        if (userProfileNav && userNameSpan && userAvatarNav) {
            userNameSpan.textContent = localStorage.getItem('username') || '';
            userAvatarNav.src = localStorage.getItem('avatar') || 'images/default-avatar.png';
    ...
    function onLoginOrRegister(email) {
        ...
        // Profil ma'lumotlarini localStorage ga saqlash
        localStorage.setItem('username', email.split('@')[0]);
        localStorage.setItem('email', email);
        localStorage.setItem('loggedIn', 'true');
        // Default avatar (agar yo'q bo'lsa)
        if (!localStorage.getItem('avatar')) {
            localStorage.setItem('avatar', 'images/default-avatar.png');
        }
        // Avtomatik profile sahifasiga o'tkazish
        window.location.href = 'profile.html';
    }
    // Profil modalini ochish
    if (userProfileNav) {
        userProfileNav.addEventListener('click', function() {
            window.location.href = 'profile.html';
        });
    }
    // Profil modalini yopish va chiqish
    const profileModal = document.getElementById('profile-modal');
    const closeProfileBtn = document.getElementById('close-profile-modal');
    const logoutBtn = document.getElementById('logout-btn');
    const avatarInput = document.getElementById('avatarInput');
    const avatarImg = document.getElementById('modalProfileAvatar');
    ...
    if (avatarInput && avatarImg) {
        ...
        // Dizayn: hover effekti
        const avatarLabel = document.getElementById('avatar-label');
        const hoverText = document.getElementById('avatar-hover-text');
        if (avatarLabel && hoverText) {
...
// --- Login/Registratsiya Modal boshqaruvi ---
document.addEventListener('DOMContentLoaded', function() {
    const modal = document.getElementById('login-register-modal');
    const openBtn = document.getElementById('login-register-btn');
    const closeBtn = document.getElementById('close-login-register');
...
// --- Anime qidiruv dropdowni (index.html uchun, faqat animeData bo'yicha) ---
document.addEventListener('DOMContentLoaded', function() {
    ...
    input.addEventListener('input', function() {
        ...
        const results = (window.animeData || animeData || []).filter(item => {
            const title = (item.title || '').toLowerCase();
            const desc = (item.description || '').toLowerCase();
            const genres = (item.genres || []).join(',').toLowerCase();
            return title.includes(val) || desc.includes(val) || genres.includes(val);
        });
        if (results.length === 0) {
            dropdown.innerHTML = '<div style="color:#fff;padding:12px 18px;">Hech narsa topilmadi</div>';
            dropdown.style.display = 'block';
            return;
        }
        dropdown.innerHTML = results.map(item =>
            `<a href="anime.html?id=${item.id}" target="_blank">
                <img src="${item.poster}" alt="${item.title}" onerror="this.src='https://placehold.co/38x38?text=Anime'">
                <div>
                  <div class="dropdown-title">${item.title}</div>
                  <div class="dropdown-short">${item.description ? item.description.substring(0, 60) + '…' : ''}</div>
                </div>
            </a>`
        ).join('');
        dropdown.style.display = 'block';
...
function renderFilters() {
    ...
    if (yearSelect) {
        // Avval "Yilni tanlang" optionini qo'shamiz
        const firstOpt = document.createElement('option');
        firstOpt.value = '';
        firstOpt.textContent = 'Yilni tanlang';
        yearSelect.appendChild(firstOpt);
        // Keyin yillarni qo'shamiz
